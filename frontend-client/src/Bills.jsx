...
function Bills() {
    ...
    const handleSave = () => {
        if (!formData.payeeName || !formData.dueDate || formData.paymentDue == '') {
            alert("Please fill out all the required fields (Payee name, Due date, and payment due) before saving.");
            return;
        }

        const method = editingId ? 'PUT' : 'POST';
        const url = editingId
            ? `${import.meta.env.VITE_API_BASE_URL}/${editingId}`
        ...
        fetch(url, {
            ...
            .then(response => response.json()).then(savedBill => {
            if (editingId) {
            ...
            fetch(`${import.meta.env.VITE_API_BASE_URL}/${id}`, {
                ...
                .then(response => {
                    if (response.ok) {
                        ...
                        alert("Something went wrong deleting the bill.");
                    }
                })
                ...
                <p>Loading bills from database...</p>
                    ...
                    {bills.map(bill => (
                        <tr key={bill.id}>
                            ...
                            <td>{bill.dueDate}</td>
                            <td>${bill.paymentDue.toFixed(2)}</td>
                            <td>
